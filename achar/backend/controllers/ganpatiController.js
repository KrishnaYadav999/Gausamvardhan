...
export const updateGanpatiProduct = async (req, res) => {
  try {
    ...
    if (req.body.keyBenefits && !Array.isArray(req.body.keyBenefits)) req.body.keyBenefits = [req.body.keyBenefits];
    if (req.body.ingredients && !Array.isArray(req.body.ingredients)) req.body.ingredients = [req.body.ingredients];
    if (req.body.packs && !Array.isArray(req.body.packs)) return res.status(400).json({ error: "Packs must be an array of {name, price}" });
    ...
    // PRICE mapping
    if (req.body.cutPrice !== undefined) req.body.cut_price = req.body.cutPrice;
    if (req.body.currentPrice !== undefined) req.body.current_price = req.body.currentPrice;
    ...
    if (req.body.title) {
      ...
      let counter = 1;
      while (await GanpatiProduct.findOne({ slug: newSlug, _id: { $ne: req.params.id } })) {
    ...
    const product = await GanpatiProduct.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!product) return res.status(404).json({ error: "Product not found" });
